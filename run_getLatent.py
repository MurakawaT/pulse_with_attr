from PULSE_getLatent import PULSE
...
# python run_getLatent.py -input_dir ../DATA/CelebAMask-HQ/CelebA-HQ-img
...
parser.add_argument('-output_dir', type=str, default='runs_check_npy_img', help='output data directory')
parser.add_argument('-latent_dir', type=str, default='latent', help='output latent directory')
...
latent_path = Path(kwargs["latent_dir"])
...
latent_path_list = glob(str(latent_path) + "/*.npy")
latent_path_list = [latent_path.replace(os.sep,'/').split("/")[-1][:-4] for latent_path in latent_path_list]
latent_path_list = set(latent_path_list)
print("already exist num : ", len(latent_path_list))


for ref_im, ref_im_name in dataloader:
    if ref_im_name[0] in latent_path_list:
        print(ref_im_name[0]+".npy : ", "already exist")
        continue
    if(kwargs["save_intermediate"]):
        ...
        for j,(HR,LR,latent_np) in enumerate(model(ref_im,**kwargs)):
            print("save ",ref_im_name)
            for i in range(kwargs["batch_size"]):
                toPIL(HR[i].cpu().detach().clamp(0, 1)).save(
                    out_path / f"{ref_im_name[i]}.png")
                np.save("{0}/{1}.npy".format(latent_path, ref_im_name[i]),latent_np)
            # if j == 2:
            #     exit()
